...
const char *RemoteControlProxy::MNOTE_SERVER_NAME = "org.mate.Mnote";
const char *RemoteControlProxy::MNOTE_SERVER_PATH = "/org/mate/Mnote/RemoteControl";
...
RemoteControlClient *RemoteControlProxy::get_instance()
{
  ...
  return new RemoteControlClient(conn, MNOTE_SERVER_PATH, MNOTE_SERVER_NAME);
}
...
RemoteControl *RemoteControlProxy::register_remote(NoteManager & manager)
{
  ...
  // using DBus-C++
  if(!conn.has_name(MNOTE_SERVER_NAME)) {
    
    conn.request_name(MNOTE_SERVER_NAME);
