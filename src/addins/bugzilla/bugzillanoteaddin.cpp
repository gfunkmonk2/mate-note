...
namespace bugzilla {
  ...
  {
    return _("Allows you to drag a Bugzilla URL from your browser directly into a Mnote note.  The bug number is inserted as a link with a little bug icon next to it.");
  }
  ...
  {
    return mnote::ADDIN_CATEGORY_DESKTOP_INTEGRATION;
  }
    ...
    : mnote::NoteAddin()
  {
    ...
    const std::string old_images_dir
      = Glib::build_filename(mnote::Mnote::old_note_dir(),
                             "BugzillaIcons");
  ...
  {
    return Glib::build_filename(mnote::Mnote::conf_dir(),
                                "BugzillaIcons");
  }
                            ...
                            const std::string & old_images_dir)
  {
    ...
    const Glib::RefPtr<Gio::File> dest
      = Gio::File::create_for_path(mnote::Mnote::conf_dir());
  ...
  bool BugzillaNoteAddin::insert_bug(int x, int y, const std::string & uri, int id)
  {
    try {
      ...
      y = y + rect.get_y();
      Gtk::TextIter cursor;
      mnote::NoteBuffer::Ptr buffer = get_buffer();
