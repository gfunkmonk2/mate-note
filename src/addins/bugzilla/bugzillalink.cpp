...
#define URI_ATTRIBUTE_NAME "uri"
  ...
  {
    s_bug_icon = mnote::utils::get_icon("bug", 16);
  }
  ...
  {
    mnote::DynamicNoteTag::initialize(element_name);
  ...
  bool BugzillaLink::on_activate(const mnote::NoteEditor & , 
                                 const Gtk::TextIter & , 
                                 const Gtk::TextIter & )
  {
    if(!get_bug_url().empty()) {
      ...
      try {
        mnote::utils::open_url(get_bug_url());
      } 
      catch (const Glib::Error & e) {
        mnote::utils::show_opening_location_error(NULL, 
                                                  get_bug_url(), e.what());
      }
    }
    return true;
  ...
  {
    mnote::DynamicNoteTag::on_attribute_read(attributeName);
