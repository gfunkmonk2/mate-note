...
{
  mnote::NoteManager & manager = mnote::Mnote::obj().default_note_manager();
  mnote::Note::Ptr template_note = manager.find(
    ...
    try {
      template_note = manager.create(
                                ...
                                NoteOfTheDay::get_template_content(
                                  NoteOfTheDay::s_template_title));
      template_note->queue_save(mnote::Note::CONTENT_CHANGED);
    }
