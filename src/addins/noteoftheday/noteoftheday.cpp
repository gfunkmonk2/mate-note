...
mnote::Note::Ptr NoteOfTheDay::create(mnote::NoteManager & manager,
                                      const Glib::Date & date)
{
  ...
  try {
    ...
    ERR_OUT("NoteOfTheDay could not create %s: %s",
            title.c_str(),
            e.what());
    return mnote::Note::Ptr();
  }

  // Automatically tag all new Note of the Day notes
  notd->add_tag(mnote::TagManager::obj().get_or_create_system_tag(
                                           "NoteOfTheDay"));

  return notd;
}

void NoteOfTheDay::cleanup_old(mnote::NoteManager & manager)
{
  mnote::Note::List kill_list;
  const mnote::Note::List & notes = manager.get_notes();

  Glib::Date date;
  date.set_time_current(); // time set to 00:00:00

  for (mnote::Note::List::const_iterator iter = notes.begin();
       notes.end() != iter; iter++) {
  ...
  for (mnote::Note::List::const_iterator iter = kill_list.begin();
                            ...
                            const mnote::NoteManager & manager)
{
  ...
  const mnote::Note::Ptr template_note = manager.find(
                                                   s_template_title);
...
mnote::Note::Ptr NoteOfTheDay::get_note_by_date(
...
bool NoteOfTheDay::has_changed(const mnote::Note::Ptr & note)
{
  ...
  return get_content_without_title(note->text_content())
           == get_content_without_title(
                mnote::utils::XmlDecoder::decode(original_xml))
         ? false
         : true;
}
