...
{
  if(!s_note_icon) {
    s_note_icon = mnote::utils::get_icon("note", 16);
  }
  return s_note_icon;
}


BacklinkMenuItem::BacklinkMenuItem(const mnote::Note::Ptr & note,
...
void BacklinkMenuItem::on_activate()
{
  ...
  mnote::NoteFindBar & find = m_note->get_window()->get_find_bar();
  find.show_all ();
