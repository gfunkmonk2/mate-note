...
void BacklinksNoteAddin::get_backlink_menu_items(std::list<BacklinkMenuItem*> & items)
{
  ...
  std::string encoded_title = sharp::string_trim(
      mnote::utils::XmlEncoder::encode(sharp::string_to_lower(search_title)));

  // Go through each note looking for
  // notes that link to this one.
  const mnote::Note::List & list = get_note()->manager().get_notes();
  for(mnote::Note::List::const_iterator iter = list.begin();
      iter != list.end(); ++iter) {
    const mnote::Note::Ptr & note(*iter);
...
bool BacklinksNoteAddin::check_note_has_match(const mnote::Note::Ptr & note, 
