...
void ExportToHtmlNoteAddin::export_button_clicked()
{
  ...
  try {
    ...
    try {
      ...
      mnote::utils::open_url("file://" + output_uri.get_absolute_uri());
    } 
      ...
      mnote::utils::HIGMessageDialog msg_dialog(
        get_window(),
  ...
  if (!error_message.empty())
  {
    ...
    mnote::utils::HIGMessageDialog msg_dialog(&dialog, 
                                              GTK_DIALOG_DESTROY_WITH_PARENT,
                                              Gtk::MESSAGE_ERROR, 
                                              Gtk::BUTTONS_OK,
                                              msg, error_message);
...
sharp::XslTransform & ExportToHtmlNoteAddin::get_note_xsl()
{
  if(s_xsl == NULL) {
    ...
    std::string stylesheet_file = DATADIR "/mnote/" STYLESHEET_NAME;
...
void ExportToHtmlNoteAddin::write_html_for_note (sharp::StreamWriter & writer, 
                                                 const mnote::Note::Ptr & note, 
                                                 bool export_linked, 
                                                 bool export_linked_all)
{
  std::string s_writer;
  s_writer = mnote::NoteArchiver::write_string(note->data());
