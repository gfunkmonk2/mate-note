...
public:
  ...
  virtual bool want_to_run(mnote::NoteManager & manager);
  virtual bool first_run(mnote::NoteManager & manager);

private:
  void check_for_first_run(mnote::NoteManager & manager);
  xmlDocPtr get_sticky_xml_doc();
  void import_button_clicked(mnote::NoteManager & manager);
  void show_no_sticky_xml_dialog(const std::string & xml_path);
  void show_results_dialog(int numNotesImported, int numNotesTotal);
  void import_notes(xmlDocPtr xml_doc, bool showResultsDialog, mnote::NoteManager & manager);
