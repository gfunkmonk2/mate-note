...
{
  return mnote::ADDIN_CATEGORY_TOOLS;
}
...
void StickyNoteImportNoteAddin::initialize()
{
  ...
  // for each note when Mnote starts up.
...
bool StickyNoteImportNoteAddin::want_to_run(mnote::NoteManager & manager)
{
  bool want_run = false;
...
bool StickyNoteImportNoteAddin::first_run(mnote::NoteManager & manager)
{
...
void StickyNoteImportNoteAddin::import_button_clicked(mnote::NoteManager & manager)
{
  xmlDocPtr xml_doc = get_sticky_xml_doc();
  if(xml_doc) {
...
{
  mnote::utils::HIGMessageDialog dialog(NULL,
