...
namespace mnote {
  ...
  void NoteManager::_common_init(const std::string & directory, const std::string & backup_directory)
  {
    ...
    const std::string old_note_dir = Mnote::old_note_dir();
  ...
  {
    return new AddinManager(Mnote::conf_dir());
  }
  ...
  void NoteManager::create_start_notes ()
  {
    ...
    std::string start_note_content =
      _("<note-content xmlns:link=\"http://beatniksoftware.com/tomboy/link\">"
        ...
        "<link:internal>Using Links in Mnote</link:internal>.  "
        "Notice how each time we type <link:internal>Using "
        "Links in Mnote</link:internal> it automatically "
        "gets underlined?  Click on the link to open the note."
        "</note-content>");

    std::string links_note_content =
      _("<note-content>"
        "Using Links in Mnote\n\n"
        "Notes in Mnote can be linked together by "
    ...
    try {
      ...
      Note::Ptr links_note = create (_("Using Links in Mnote"),
                                links_note_content);
