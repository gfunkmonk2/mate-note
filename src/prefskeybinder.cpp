...
namespace mnote {
  ...
  PrefsKeybinder::PrefsKeybinder()
    : m_native_keybinder(Mnote::obj().keybinder())
  {
  }
  ...
  MnotePrefsKeybinder::MnotePrefsKeybinder(NoteManager & manager, IMnoteTray & trayicon)
    : m_manager(manager)
    , m_trayicon(trayicon)
  {
    enable_disable(Preferences::obj().get<bool>(Preferences::ENABLE_KEYBINDINGS));
    m_prefs_cid = Preferences::obj().signal_setting_changed()
      .connect(sigc::mem_fun(*this, &MnotePrefsKeybinder::enable_keybindings_changed));
  }


  MnotePrefsKeybinder::~MnotePrefsKeybinder()
  {
    m_prefs_cid.disconnect();
  }


  void MnotePrefsKeybinder::enable_keybindings_changed(Preferences*, MateConfEntry* entry)
  {
  ...
  void MnotePrefsKeybinder::enable_disable(bool enable)
  {
    DBG_OUT("EnableDisable Called: enabling... %s", enable ? "true" : "false");
    if(enable) {
      bind_preference (Preferences::KEYBINDING_SHOW_NOTE_MENU,
                       sigc::mem_fun(*this, &MnotePrefsKeybinder::key_show_menu));

      bind_preference (Preferences::KEYBINDING_OPEN_START_HERE,
                       sigc::mem_fun(*this, &MnotePrefsKeybinder::key_openstart_here));

      bind_preference (Preferences::KEYBINDING_CREATE_NEW_NOTE,
                       sigc::mem_fun(*this, &MnotePrefsKeybinder::key_create_new_note));

      bind_preference (Preferences::KEYBINDING_OPEN_SEARCH,
                       sigc::mem_fun(*this, &MnotePrefsKeybinder::key_open_search));

      bind_preference (Preferences::KEYBINDING_OPEN_RECENT_CHANGES,
                       sigc::mem_fun(*this, &MnotePrefsKeybinder::key_open_recent_changes));
    }
    else {
      unbind_all();
    }
  ...
  void MnotePrefsKeybinder::bind_preference(const std::string & pref_path, const sigc::slot<void> & handler)
  {
    bind(pref_path, Preferences::obj().get_default<std::string>(pref_path), handler);
  }


  void MnotePrefsKeybinder::key_show_menu()
  {
  ...
  void MnotePrefsKeybinder::key_openstart_here()
  {
  ...
  void MnotePrefsKeybinder::key_create_new_note()
  {
    try {
  ...
  void MnotePrefsKeybinder::key_open_search()
  {
  ...
  void MnotePrefsKeybinder::key_open_recent_changes()
  {
