...
namespace mnote {
  ...
  Tray::Tray(NoteManager & manager, IMnoteTray & trayicon)
    : m_manager(manager)
    , m_trayicon(trayicon)
    , m_menu_added(false)
  {
...
//        m_tray_menu->attach_to_widget(*parent);
  ...
  TrayIcon::TrayIcon(NoteManager & manager)
    ...
    , m_keybinder(new MnotePrefsKeybinder(manager, *this))
    , m_context_menu(NULL)
  {
  ...
  Gtk::Menu * TrayIcon::get_right_click_menu()
  {
    ...
    item = manage(new Gtk::ImageMenuItem(_("_About Mnote"), true));
...
//      get_geometry(screen, area, orientation);
  ...
  bool TrayIcon::on_size_changed(int size)
  {
    ...
    Glib::RefPtr<Gdk::Pixbuf> pixbuf = utils::get_icon("mnote", size);
    set(pixbuf);
                                                ...
                                                const std::string & mateconf_path)
  {
    ...
    if(Mnote::obj().keybinder().get_accel_keys(mateconf_path, keyval, mods)) {
