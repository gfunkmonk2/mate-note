...
namespace mnote {
  ...
  int Mnote::main(int argc, char **argv)
  {
    MnoteCommandLine cmd_line;
    ...
    m_icon_theme->append_search_path(DATADIR"/mnote/icons");

    std::string note_path = get_note_path(cmd_line.note_path());
    m_manager = new NoteManager(note_path, sigc::mem_fun(*this, &Mnote::start_note_created));
...
#if HAVE_MATEPANELAPPLET
      Mate::Panel::init("mnote", VERSION, argc, argv);
  ...
  void Mnote::start_note_created(const Note::Ptr & start_note)
  {
  ...
  std::string Mnote::get_note_path(const std::string & override_path)
  {
    std::string note_path;
    if(override_path.empty()) {
      const char * s = getenv("MNOTE_PATH");
      note_path = s?s:"";
    }
    else {
    ...
    if(note_path.empty()) {
      note_path = Mnote::data_dir();
    }

    return note_path;
  }

  void Mnote::start_tray_icon()
  {
    ...
    if (!is_tray_icon_showing) {
      ...
      timeout->connect(sigc::mem_fun(*this, &Mnote::check_tray_icon_showing));
      timeout->attach();
    }
  ...
  bool Mnote::check_tray_icon_showing()
  {
    bool is_tray_icon_showing = m_tray_icon->is_embedded()
      && m_tray_icon->get_visible();
    if(!is_tray_icon_showing) {
  ...
  void Mnote::register_remote_control(NoteManager & manager)
  {
#if ENABLE_DBUS
    try {
      m_remote_control = RemoteControlProxy::register_remote(manager);
      if (m_remote_control) {
        DBG_OUT("Mnote remote control active.");
      } 
      else {
        // If Mnote is already running, open the search window
        // so the user gets some sort of feedback when they
        // attempt to run Mnote again.
        ...
        ERR_OUT ("Mnote is already running.  Exiting...");
        ::exit(-1);
      }
    } 
    catch (const std::exception & e) {
      ERR_OUT("Mnote remote control disabled (DBus exception): %s",
              e.what());
    }
#endif
  }
  ...
  void Mnote::setup_global_actions()
  {
    ActionManager & am(ActionManager::obj());
    am["NewNoteAction"]->signal_activate()
      .connect(sigc::mem_fun(*this, &Mnote::on_new_note_action));
    am["QuitGNoteAction"]->signal_activate()
      .connect(sigc::mem_fun(*this, &Mnote::on_quit_mnote_action));
    am["ShowPreferencesAction"]->signal_activate() 
      .connect(sigc::mem_fun(*this, &Mnote::on_show_preferences_action));
    am["ShowHelpAction"]->signal_activate()
      .connect(sigc::mem_fun(*this, &Mnote::on_show_help_action));
    am["ShowAboutAction"]->signal_activate()
      .connect(sigc::mem_fun(*this, &Mnote::on_show_about_action));
    am["TrayNewNoteAction"]->signal_activate()
      .connect(sigc::mem_fun(*this, &Mnote::on_new_note_action));
    am["ShowSearchAllNotesAction"]->signal_activate()
      .connect(sigc::mem_fun(*this, &Mnote::open_search_all));
    am["NoteSynchronizationAction"]->signal_activate()
      .connect(sigc::mem_fun(*this, &Mnote::open_note_sync_window));
  }

  void Mnote::on_new_note_action()
  {
    try {
  ...
  void Mnote::on_quit_mnote_action()
  {
    if(m_is_mate_panel_applet) {
      return;
    }
  ...
  void Mnote::on_preferences_response(int /*res*/)
  {
    delete m_prefsdlg;
    m_prefsdlg = NULL;
  }
  ...
  void Mnote::on_show_preferences_action()
  {
    if(!m_prefsdlg) {
      m_prefsdlg = new PreferencesDialog(m_manager->get_addin_manager());
      m_prefsdlg->signal_response().connect(
        sigc::mem_fun(*this, &Mnote::on_preferences_response));
    }
    m_prefsdlg->present();
  }

  void Mnote::on_show_help_action()
  {
    ...
    utils::show_help("mnote", "", cscreen, NULL);
  }

  void Mnote::on_show_about_action()
  {
    ...
    about.set_name("Mnote");
    about.set_version(VERSION);
    about.set_logo(utils::get_icon("mnote", 48));
...
//        }
//      }); 
    about.set_website("http://mate-desktop.org/");
    about.set_website_label(_("MATE Desktop Webpage"));
    about.set_authors(authors);
    about.set_documenters(documenters);
    about.set_translator_credits(translators);
//      about.set_icon_name("mnote");
  ...
  std::string Mnote::cache_dir()
  {
    return Glib::get_user_cache_dir() + "/mnote";
  }


  std::string Mnote::conf_dir()
  {
    return Glib::get_user_config_dir() + "/mnote";
  }


  std::string Mnote::data_dir()
  {
    return Glib::get_user_data_dir() + "/mnote";
  }


  std::string Mnote::old_note_dir()
  {
    ...
    return home_dir + "/.mnote";
  }



  MnoteCommandLine::MnoteCommandLine()
    ...
    , m_highlight_search(NULL)
  {
    static const GOptionEntry entries[] =
      {
        { "mate-panel-applet", 0, G_OPTION_FLAG_HIDDEN, G_OPTION_ARG_NONE, &m_use_panel, _("Run Mnote as a MATE panel applet."), NULL },
        { "note-path", 0, 0, G_OPTION_ARG_STRING, &m_note_path, _("Specify the path of the directory containing the notes."), _("path") },
        { "search", 0, G_OPTION_FLAG_OPTIONAL_ARG, G_OPTION_ARG_CALLBACK, (void*)MnoteCommandLine::parse_func, _("Open the search all notes window with the search text."), _("text") },
        { "version", 0, 0, G_OPTION_ARG_NONE, &m_show_version, _("Print version information."), NULL },
#ifdef ENABLE_DBUS
        { "new-note", 0, G_OPTION_FLAG_OPTIONAL_ARG, G_OPTION_ARG_CALLBACK, (void*)MnoteCommandLine::parse_func, _("Create and display a new note, with a optional title."), _("title") },
    ...
    GOptionGroup *group = g_option_group_new("Mnote", _("A note taking application"), _("Mnote options at launch"), this, NULL);
  ...
  gboolean MnoteCommandLine::parse_func(const gchar *option_name,
                                        const gchar *value,
                                        gpointer data,
                                        GError ** /*error*/)
  {
    MnoteCommandLine * self = (MnoteCommandLine*)data;
  ...
  void MnoteCommandLine::parse(int &argc, gchar ** & argv)
  {
    GError *error = NULL;
    ...
    if(m_open_note && *m_open_note) {
      if (Glib::str_has_prefix(m_open_note, "note://mnote/")) {
...

  {
    ...
    if (!m_open_external_note_path.empty()) {
      ...
      if (!note_id.empty()) {
        ...
        if (!display_note(remote, str(boost::format("note://mnote/%1%") % note_id))) {
    ...
    if(m_do_search) {
      NoteRecentChanges * recent_changes
        = NoteRecentChanges::get_instance(
          Mnote::obj().default_note_manager());
  ...
  void MnoteCommandLine::print_version()
  {
  ...
  bool MnoteCommandLine::display_note(RemoteControlClient * remote,
                                      std::string uri)
  {
#ifdef ENABLE_DBUS
  ...
  bool MnoteCommandLine::needs_execute() const
  {
