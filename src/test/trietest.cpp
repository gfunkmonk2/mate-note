...
int test_main(int /*argc*/, char ** /*argv*/)
{
  ...
  mnote::TrieTree<std::string> trie(false);
  ...
  mnote::TrieTree<std::string>::HitListPtr matches(trie.find_matches (src));
  BOOST_CHECK( matches.get() );

  BOOST_CHECK( matches->size() == 16 );
  mnote::TrieTree<std::string>::HitList::const_iterator iter = matches->begin();
  ...
  for(mnote::TrieTree<std::string>::HitList::const_iterator hit_iter = matches->begin();
      hit_iter != matches->end(); ++hit_iter) {
    mnote::TrieHit<std::string> *hit(*hit_iter);
