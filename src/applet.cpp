...
#define IID "OAFIID:MnoteApplet"
#define FACTORY_IID "OAFIID:MnoteApplet_Factory"
...
class MnoteMatePanelAppletEventBox
  : public Gtk::EventBox
  , public mnote::IMnoteTray
{
public:
  MnoteMatePanelAppletEventBox(NoteManager & manager);
  virtual ~MnoteMatePanelAppletEventBox();
...
MnoteMatePanelAppletEventBox::MnoteMatePanelAppletEventBox(NoteManager & manager)
  : m_manager(manager)
  , m_tray(new Tray(manager, *this))
  , m_panel_size(16)
  , m_image(utils::get_icon("mnote", m_panel_size))
{
  property_can_focus() = true;
  signal_button_press_event().connect(sigc::mem_fun(
                                        *this, &MnoteMatePanelAppletEventBox::button_press));
  add(m_image);
  show_all();
...
MnoteMatePanelAppletEventBox::~MnoteMatePanelAppletEventBox()
{
}


bool MnoteMatePanelAppletEventBox::button_press(GdkEventButton *ev)
{
...
void MnoteMatePanelAppletEventBox::prepend_timestamped_text(const Note::Ptr & note,
                                                        const sharp::DateTime & timestamp,
                                                        const std::string & text)
{
...
bool MnoteMatePanelAppletEventBox::paste_primary_clipboard()
{
...
void MnoteMatePanelAppletEventBox::show_menu(bool select_first_item)
{
...
void MnoteMatePanelAppletEventBox::setup_drag_and_drop()
{
  ...
  signal_drag_data_received().connect(
    sigc::mem_fun(*this, &MnoteMatePanelAppletEventBox::on_drag_data_received));
}

void MnoteMatePanelAppletEventBox::on_drag_data_received(const Glib::RefPtr<Gdk::DragContext>&,
                                                     int,int,
...
void MnoteMatePanelAppletEventBox::init_pixbuf()
{
  ...
  // info.
  if (Mnote::obj().is_mate_panel_applet())
  ...
  Glib::RefPtr<Gdk::Pixbuf> new_icon = utils::get_icon("mnote", icon_size);
  m_image.property_pixbuf() = new_icon;
}


PanelOrientation MnoteMatePanelAppletEventBox::get_panel_orientation()
{
...
void MnoteMatePanelAppletEventBox::on_size_allocate(Gtk::Allocation& allocation)
{
...
bool MnoteMatePanelAppletEventBox::menu_opens_upward()
{
...
{
public:
  explicit MnoteApplet(MatePanelApplet *);
  virtual ~MnoteApplet();
...
private:
  ...
  NoteManager & m_manager;
  MnoteMatePanelAppletEventBox m_applet_event_box;
  PrefsKeybinder *m_keybinder;
};


MnoteApplet::MnoteApplet(MatePanelApplet *castitem)
  : Mate::Panel::Applet(castitem)
  , m_manager(Mnote::obj().default_note_manager())
  , m_applet_event_box(m_manager)
  , m_keybinder(new MnotePrefsKeybinder(m_manager, m_applet_event_box))
{
  static const MateComponentUIVerb menu_verbs[] = {
    MATECOMPONENT_UI_VERB("Props", &MnoteApplet::show_preferences_verb),
    MATECOMPONENT_UI_VERB("Help", &MnoteApplet::show_help_verb),
    MATECOMPONENT_UI_VERB("About", &MnoteApplet::show_about_verb),
    MATECOMPONENT_UI_VERB_END
  };

  setup_menu(DATADIR"/mnote", "MATE_MnoteApplet.xml", "", menu_verbs, this);

  add (m_applet_event_box);
  Mnote::obj().set_tray(m_applet_event_box.get_tray());
...
MnoteApplet::~MnoteApplet()
{
  delete m_keybinder;
}

void MnoteApplet::show_preferences_verb(MateComponentUIComponent*, void*, const char*)
{
  ActionManager::obj()["ShowPreferencesAction"]->activate();
}

void MnoteApplet::show_help_verb(MateComponentUIComponent*, void* data, const char*)
{
  MnoteApplet* applet = static_cast<MnoteApplet*>(data);
  // Don't use the ActionManager in this case because
  // the handler won't know about the Screen.
  utils::show_help("mnote", "", applet->get_screen()->gobj(), NULL);
}

void MnoteApplet::show_about_verb(MateComponentUIComponent*, void*, const char*)
{
  ActionManager::obj()["ShowAboutAction"]->activate();
}

  void MnoteApplet::on_change_background(Mate::Panel::AppletBackgroundType type, 
...
void MnoteApplet::on_change_size(int size)
{
...
int register_applet()
{
  int returncode = Mate::Panel::factory_main<MnoteApplet>(FACTORY_IID);
  return returncode;
}
